...
namespace utopicsense 
{
    public class UWindow : EditorWindow
    {
        ...
        public void Awake()
        {
            ...
            utopicSenseVisualScript = (UtopicSenseVisual)AssetDatabase.LoadAssetAtPath("PrefabUS/UtopicSense.asset", typeof(UtopicSenseVisual)); //checkbefore

            if (utopicSenseVisualScript != null)
            {
                this.colors = utopicSenseVisualScript.colors;
                this.simplified = utopicSenseVisualScript.simplified;
                if (this.simplified == true)
                {
                    selection = 1;
                }
                this.selectionShader = utopicSenseVisualScript.typeShader;
            }
            else
            {
                createUtopicSense();
                putToDefaultColors();
                utopicSenseVisualScript.colors = this.colors;
                utopicSenseVisualScript.simplified = this.simplified;
                if (shaderOptions != null)
                    utopicSenseVisualScript.typeShader = this.selectionShader;
                else
                    utopicSenseVisualScript.typeShader = -1;
            }
        ...
        private void OnGUI()
        {
            ...
            if (selection == 1)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    Debug.Log("MUDEI ALGUMA COR E DEVO SALVAR");
                    utopicSenseVisualScript.colors = this.colors;
                    utopicSenseVisualScript.simplified = this.simplified;
                    AssetDatabase.SaveAssets();
                    AssetDatabase.Refresh();
        ...
        private void createUtopicSense()
        {
            ...
            AssetDatabase.CreateAsset(utopicSenseVisualScript, "Assets/UtopicSense/Resources/PrefabUS/UtopicSense.asset");
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            EditorUtility.SetDirty(utopicSenseVisualScript);
        }
        ...
        private void deactivateUtopicSenseInCurrentScene()
        {
            ...
            SoundEffect[] effectsUsed = FindObjectsOfType<SoundEffect>();
